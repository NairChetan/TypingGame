document.addEventListener('DOMContentLoaded', () => {
    ...
    function startGame() {
        difficultySelection.classList.add('d-none'); // hiding the difficulty options
        gameContainer.classList.remove('d-none'); // makes the game container visible
 
        gameEnded = false; // set to game is active
 
        intervalId = setInterval(() => {
            if (fallenLetters.length >= letters.length || gameEnded) { // checks if the number of fallen letters is equal to or greater than the total number of letters, or if the game has ended
                clearInterval(intervalId); // This line stops the repeated execution of the interval function by clearing the interval using intervalId
                setTimeout(endGame, 2000); // This line schedules the endGame function to be called after a 2-second delay
                return;
            }
        ...
        startTime = Date.now(); // used to get the current time
    ...
    function showResult() {
        ...
        // Hide pressed, missed, and wrong counts
    ...
    function updatePressedCount() {
        pressedCountDisplay.textContent = `Pressed: ${pressedCount}`;
    }
    ...
    function resetGame() {
        ...
        fallenLetters = [];
        gameEnded = false;
        // Show pressed, missed, and wrong counts
        pressedCountDisplay.style.display = 'block';
        missedCountDisplay.style.display = 'block';
        wrongCountDisplay.style.display = 'block';
    }
